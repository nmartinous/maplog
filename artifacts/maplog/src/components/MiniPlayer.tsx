...
export function MiniPlayer() {
  ...
  /** Seek by clicking/tapping anywhere on the scrubber track */
  ...
  if (!currentSong) {
    return (
      <div className="fixed bottom-16 sm:bottom-0 left-0 right-0 h-16 bg-card/80 backdrop-blur-md border-t border-border z-40 sm:z-50 px-5 flex items-center justify-between select-none">
        ...
        {/* Ghost controls — keep visual weight consistent */}
  ...
  // ── Active state ────────────────────────────────────────────────────────────
  const artwork = (currentSong as any).cards?.[0]?.artworkUrl as string | undefined;
  ...
  return (
    <div className="fixed bottom-16 sm:bottom-0 left-0 right-0 h-16 bg-card/92 backdrop-blur-md border-t border-border z-40 sm:z-50 flex flex-col">
      ...
      {/* ── Interactive scrubber — full width, tappable ────────────────────── */}
      <div
        ref={barRef}
      ...
      >
        ...
        />
        {/* Scrub handle — appears on hover/active */}
        <div
        ...
        />
        {/* Invisible tap target — makes the 4px bar much easier to hit on mobile */}
      ...
      <div className="flex-1 flex items-center px-3 gap-2 min-w-0">
        ...
        {/* Artwork — tapping opens the full Now Playing screen */}
        <Link href="/" className="shrink-0 cursor-pointer">
          <div className="w-9 h-9 rounded-md bg-muted overflow-hidden">
            {artwork ? (
              <img src={artwork} alt={currentSong.title} className="w-full h-full object-cover" />
            ) : (
        ...
        {/* Title + time — tapping title opens Now Playing */}
