...
import { Library, ListMusic, ListOrdered, Volume2, ChevronDown } from 'lucide-react';
...
export default function Home() {
  ...
  // currentSong already has cards attached — no extra fetch needed
    ...
    <div className="h-[calc(100dvh-4rem)] sm:h-[100dvh] bg-black text-white flex flex-col relative">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        {cards[activeCardIndex]?.artworkUrl ? (
          ...
          <img src={currentSong.artworkUrl} alt="" className="w-full h-full object-cover blur-3xl scale-150 opacity-30" />
        ) : (
      ...
      <div className="relative z-10 flex items-center justify-between px-6 pt-10 pb-2 shrink-0">
        <Button variant="ghost" size="icon" className="w-9 h-9 rounded-full text-white/50 hover:text-white hover:bg-white/10" asChild>
          <Link href="/collection"><ChevronDown className="h-5 w-5" /></Link>
        </Button>
        <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-white/40">Now Playing</span>
        <Button variant="ghost" size="icon" className="w-9 h-9 rounded-full text-white/50 hover:text-white hover:bg-white/10" onClick={() => setIsQueueOpen(true)}>
          ...
          <>
            ...
            {cards.length > 1 && (
              <div className="flex items-center justify-center gap-2 shrink-0">
                {cards.map((_, i) => (
                  <div key={i} className={cn('h-1 rounded-full transition-all duration-300', i === activeCardIndex ? 'w-5 bg-white' : 'w-1 bg-white/25')} />
                ))}
              </div>
            )}
          </>
      ...
      {isQueueOpen && (
        <div className="absolute inset-0 z-50 bg-black/85 backdrop-blur-2xl flex flex-col">
          <div className="flex justify-between items-center px-6 pt-12 pb-4 shrink-0">
            ...
            <Button variant="ghost" size="sm" className="rounded-full px-4 border border-white/15 text-white/70 hover:text-white" onClick={() => setIsQueueOpen(false)}>
              Done
            </Button>
          </div>
          <div className="flex-1 overflow-y-auto px-4 pb-8">
            {queue.length === 0 ? (
              <div className="text-center py-24"><p className="text-white/25 text-sm">Queue is empty</p></div>
            ) : (
              <div className="space-y-0.5">
                {queue.map((song, i) => {
                  const isCurrent = song.id === currentSong?.id;
                  return (
                    <div key={`${song.id}-${i}`} className={cn('flex items-center gap-3 px-3 py-3 rounded-xl transition-colors', isCurrent ? 'bg-white/12' : 'hover:bg-white/5')}>
                      <div className={cn('w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-xs font-mono', isCurrent ? 'bg-white text-black font-bold' : 'bg-white/8 text-white/40')}>
                      ...
                      {isCurrent && isPlaying && (
                        <div className="shrink-0 flex items-end gap-[2px] h-4">
                          {[0, 1, 2].map(j => (
                            <div key={j} className="w-[3px] bg-white rounded-full animate-[bounce_0.9s_ease-in-out_infinite]"
