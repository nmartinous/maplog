...
export default function Playlists() {
  ...
  return (
    // h-full + overflow-y-auto = scrolls within the app shell
    <div className="h-full overflow-y-auto">
      <div className="px-4 sm:px-6 pt-5 pb-4">
        <h1 className="text-2xl font-extrabold tracking-tight">By Rarity</h1>
        <p className="text-xs text-muted-foreground mt-0.5">Your collection grouped by card tier</p>
      </div>
      ...
      {groups.length === 0 ? (
        ...
        <div className="px-4 sm:px-6 pb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="group bg-card hover:bg-accent border border-border rounded-2xl p-5 transition-all cursor-pointer active:scale-[0.98]">
                <div className="flex gap-1.5 mb-4 h-14 items-end">
                  {groupSongs.slice(0, 3).map((song, i) => (
                    <div key={song.id}
                      className={cn('rounded-xl overflow-hidden shrink-0', i === 0 ? 'w-14 h-14' : 'w-10 h-10 opacity-60')}>
                      {song.artworkUrl
